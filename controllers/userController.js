...
export const getRandomUnconnectedUsers = async (req, res) => {
    try {
        const { userId } = req.auth(); // Logged-in user ID

        // Get current user's connections
        const currentUser = await User.findById(userId, { connections: 1 });
        if (!currentUser) {
        ...
        // Find all users who are NOT you and NOT in your connections
        ...
        // Limit to max 3
        const limitedUsers = shuffled.slice(0, Math.min(3, unconnectedUsers.length));
